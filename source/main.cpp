...
int main(int argc, char *argv[])
{
    ...
    else if (gameType == "langton") 
    {
        ...
        sf::Vector2u resolution(aspectRatio * 20u);
        App app(resolution.x, resolution.y, 60, "Langton Ant");
        auto& langton = app.SetCellularAutomaton<cellular::langton::Langton>(resolution.x, resolution.y);
        auto& ant = langton.AddAnt<cellular::langton::NormalLangtonAnt>();
        ant.SetPosition(resolution.x/2.0f, resolution.y/2.0f);
        app.Run();
    }
    else
    {
